...
public class ApiController {
    ...
    public Result uploadImage(MultipartFile image){
        ...
        //获取当前年月以创建目录
        String workPath=this.getClass().getResource("/static").getPath();
        String parentPath=configService.findByName(ConfigNameEnum.IMAGE_HOME.name());
        log.info(workPath);
        log.info(parentPath);
        log.info(parentPath=workPath+"/image");
        File mediaPath = new File(parentPath, savePath);
        if (!mediaPath.exists()) {
            mediaPath.mkdirs();
        }
//        log.info(mediaPath.getAbsolutePath());
        //以当前系统时间作为文件名
...
//        System.out.println(mediaPath.getAbsolutePath());
        try {
